...
mds_plot <- mds %>%
  ...
  xlab("MDS1") +
  ylab("MDS2") +
  scale_color_manual(values = c("#c4c4c4", "#cf2828")) +
  theme_cowplot() +
...
perform_regression <- function(dat) {
  ...
  # Multivariate logistic regression routine over 10,000 iterations to select
  ...
  # 2,000 of the 10,000 iterations)
  coef_matrix <- as.matrix(regression_df[-1][, idx])
  rownames(coef_matrix) <- rownames(dat)
...
perform_lca <- function(dat) {
  ...
  estimate_blca_res <- function(mut_mat, num_classes) {
    ...
    local_mut_mat <- mut_mat[sample(seq_len(nrow(mut_mat)),
    ...
    tryCatch(expr = {
      ...
      local_blca_res <- map(num_classes,
                            ~ blca(X = local_mut_mat,
                                   G = .x,
                                   method = "em",
                                   restarts = 5,
                                   start.vals = "across"))

      best_k <- which.max(map_dbl(local_blca_res,
                                  ~ .x$BIC))
      ...
      if ((nrow(best_res$Z) == nrow(local_mut_mat)) &
        ...
        cluster_assign <- setNames(map_int(1:nrow(best_res$Z),
        ...
        cluster_assign <- setNames(map_int(1:nrow(best_res$Z),
                                           ~ which.max(best_res$Z[.x, ]))
                                   [match(map_chr(1:nrow(local_mut_mat),
  ...
  for(r in 1:length(blca_res_em)) {
    ...
    local_clust <- as.matrix(Matrix::Diagonal(n = 2)[rep(1,
                                                         length(local_res)), ])
    local_clust <- local_clust %*% t(local_clust)
    idx <- match(sorted_ids, table = names(local_res))
    local_clust <- local_clust[idx, idx]
