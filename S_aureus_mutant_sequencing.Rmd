...
gene-level mutations in each evolutionary replicate. We use this data frame for
subsequent analyses.

```{r Generation of binary data frame of mutations}
generate_binary_matrix <- function(dat) {
  ...
  colnames(binary_mat) <- populations
  rownames(binary_mat) <- genes

  # Use the qualifying_mutations object to populate the binary_mat object.
  # This code chunk iterates through the mutations data frame and populates the
  # binary_mat object with a 1 if a mutation is present in a
  # given gene / lineage combination
  for (i in seq_len(nrow(dat))) {
    binary_mat[dat$gene[i], dat$population[i]] <- 1
  }
...
By default, the GenomicAnalysis function will output a list containing two
elements:
...
perform_genomic_analysis <- function(dat, bootstrap = FALSE) {
  if (bootstrap == TRUE) {
    ...
    dat <- dat %>%
      as_tibble() %>%
      sample(size = ncol(dat), replace = TRUE)

    # Rename the columns of the sampled dataset - some columns are sampled
    # more than once. By default R will append a number to the end of the
    # column name to differentiate between these columns. These columns are then
    # treated separately in the downstream analyses. This code chunk renames
    # the columns to C1, C2, ..., C87 and E1, E2, ..., E18 etc
    # to avoid this issue.

    colnames(dat) <- c(paste0("C", 1:87), paste0("E", 1:18))
  }
  ...
  # If the GenomicAnalysis function is told to perform a bootstrap analysis,
  # then only the difference is returned. The significance of this statistic
  # is then calculated.
  if (bootstrap == FALSE) {
...
perform_bootstrap_analysis <- function(dat) {
  # This function iterates through the GenomicAnalysis function 10,000 times
  # and stores the results in a data frame. The function then returns the
  # data frame.
  plan(multisession)
  ...
  results <- future_map_dfr(1:num_trials,
...
associated with each treatment tratment. To identify these genes, we perform
...
regression_analysis <- function(dat) {
  coef_count <- rep(0, ncol(dat[-1]))

  # Multivariate logistic regression routine over 100 iterations to select more
  # robust non-zero coefficients
  for (i in 1:100) {
    cvfit <- cv.glmnet(
      as.matrix(dat[, -1]),
      as.factor(dat$treatment),
  ...
  rownames(coef_matrix) <- colnames(binary_matrix)
...
coef_matrix <- regression_analysis(regression_df)

# Heat map of control populations
...
```{r Mutation heat map}
generate_boxplot_clusters <- function(dat) {
  # Factor antibiotics by order of decreasing median collateral response
  antibiotic_factor <- dat %>%
    group_by(antibiotic) %>%
    summarize(med = median(mean_CR)) %>%
  ...
  CR_boxplot <- dat %>%
    ...
    geom_boxplot() +
    # Color and jitter points associated with populations E4, E5, E6, E7, E16,
    # and E18.
    geom_jitter(
      data = dat %>%
        filter(
          population %in% c("E4", "E5", "E6", "E7", "E16", "E18")
        ),
      shape = 16,
      position = position_jitter(0.2),
      size = 2,
    ...
    geom_jitter(
      data = dat %>%
        filter(population %in% c("E1", "E2", "E3", "E11")),
      shape = 16,
      position = position_jitter(0.2),
      size = 2,
      color = "#00BA38"
...
CR_cluster_boxplot <- generate_boxplot_clusters(summary_stats)

CR_cluster_boxplot

# ggsave("CR_Cluster_boxplot.pdf",
#        CR_Cluster_boxplot,
#        path = "Figures",
#        device = "pdf",
#        width = 10,
